...
PATH = os.getcwd()
data_path = "/".join(PATH,"dataset/labeled_tweets.csv")
stopword_corpus = "/".join(PATH,"static/en_stopwords.txt")
custom_stopword_corpus = "/".join(PATH,"static/en_stopwords_viz.txt")
processed_data_path = "/".join(PATH,"dataset/cleaned_tweets.csv")
tokenizer_path = "/".join(PATH,"static/tokenizer.pickle")
saved_model_path = "/".join(PATH,"static/lstm_model.h5")
...
def text_preprocessing(text):
    stopwords = set()
    with open(stopword_corpus, "r") as file:
        for word in file:
...
def plot_wordcloud(tweet_df, colormap="Greens"):
    stopwords = set()
    with open(custom_stopword_corpus, "r") as file:
        for word in file:
            stopwords.add(word.rstrip("\n"))
